...
class TcAdsSOAP
{
	...
	public short ReadInt(String netId, int port, long group, long offset) throws Exception 
	{			
	...
	public boolean ReadBool(String netId, int port, long group, long offset) throws Exception 
	{
		DataInputStream data = null;
	...
	public String ReadString(String netId, int port, long group, long offset, int cblen) throws Exception 
	{			
		String ret="";
		DataInputStream data = null;
	...
	public boolean WriteInt(String netId, int port, long group, long offset, int data)
	{
		...
		{	
			pData[(cblen-1)-i] = (byte)((t2b(hexs.charAt(i*2))*16) + t2b(hexs.charAt((i*2)+1)));
		}
				
		return SOAPWrite(netId, port, group, offset, pData);
	}
	
	public boolean WriteString(String netId, int port, long group, long offset, String data)
	{
		return SOAPWrite(netId, port, group, offset, data.getBytes());
	}
	
	public boolean WriteBool(String netId, int port, long group, long offset, boolean data)
	{
		byte[] x = new byte[1];
		if (data)
			x[0]='1';
	...
	private boolean SOAPWrite(String netId, int port, long group, long offset, byte[] data)
	{
	...
	private DataInputStream SOAPCall(String netId, int port, long group, long offset, int cblen)
	{
	...
	private DataInputStream SOAPCall(String netId, int port, long group, long offset, int cblen, boolean swap)
	{
		...
		try
		{
			...
			wout.write(        "<indexGroup xsi:type=\"xsd:unsignedInt\">"+group+"</indexGroup>");
			wout.write(        "<indexOffset xsi:type=\"xsd:unsignedInt\">"+offset+"</indexOffset>");
